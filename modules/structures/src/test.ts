import { Bool, combine, restrict, Val } from "./arrows";
...
list.pipe().tee(3).map([
    ...
    pipe => pipe.take(2, 1).flat().tee().map([
        pipe => pipe.apply(obj => obj.register((val) => console.log("Log:", val)).leak().count().apply(console.log)).max()
            .pipe().zip(Pipe.of("Max:").expand(val => val[0]).for(30).flat()).flat().reverse().collect().map(val => val.join(" ")),
...
Pipe.of(0, 1).buffer().expand(prev => prev.at(-1)! + prev.at(-2)!).while(val => val && val < Number.POSITIVE_INFINITY).flat().tee().map([
    pipe => pipe.max(),
    pipe => pipe.count()
